...
//  Parallax Background Image
    ...
    $(".ts-form-email .btn[type='submit']").on("click", function(e){
        ...
        var pathToPhp = $(this).closest("form").attr("action");
        $form.validate({
            submitHandler: function() {
                $button.addClass("processing");
                $.post( pathToPhp, $form.serialize(),  function(response) {
                    $button.addClass("done").find(".status").html("<i class='fas fa-circle-notch fa-spin spinner'></i>");
                    $button.removeClass("processing");
                    $(".form-contact-status").html(response).slideDown("fast");
                });
                return false;
            }
        });
...
function heroHeight() {
    $(".ts-full-screen").height($(window).height()+50);
}
